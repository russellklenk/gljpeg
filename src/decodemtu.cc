...
/// the asynchronous I/O system with unbuffered/uncached I/O.
///////////////////////////////////////////////////////////////////////////80*/
...
//   Data Types   //
//////////////////*/
struct UCIO_ALLOCATOR
{
    uint8_t          *BaseAddress;
    size_t            NextAllocation;
    size_t            AllocatorCapacity;
...
CalculateAllocatorSize
(
    int64_t file_size
)
{   // assume for now that sector size == page size == 4096.
    return AlignUp((size_t) file_size, 4096);
}

internal_function size_t
CalculateFileDataLoadSize
(
...
)
{
    ...
    {
        flsize += CalculateAllocatorSize(file_list[i].FileSize);
    }
    return flsize;
}

internal_function int
CreateUncachedIoAllocator
(
    UCIO_ALLOCATOR *allocator, 
...
)
{
    ...
    ZeroMemory(allocator, sizeof(UCIO_ALLOCATOR));

    if ((mem_base = VirtualAlloc(NULL, allocator_capacity, MEM_RESERVE|MEM_COMMIT, PAGE_READWRITE)) == NULL)
    {
        ConsoleError("ERROR: %S(%u): VirtualAlloc for %Iu bytes failed (%08X).\n", __FUNCTION__, GetCurrentThreadId(), allocator_capacity, GetLastError());
        return -1;
    }
    // TODO(rlk): touch all pages to wire them?
    allocator->BaseAddress       =(uint8_t*) mem_base;
    allocator->NextAllocation    = 0;
    allocator->AllocatorCapacity = allocator_capacity;
    InitializeCriticalSectionAndSpinCount(&allocator->AllocatorLock, 0x1000);
    return 0;
}
...
{
    size_t alloc_size = CalculateAllocatorSize(file_size);
    void  *alloc_addr = NULL;
    EnterCriticalSection(&allocator->AllocatorLock);
    {
        if (allocator->NextAllocation + alloc_size <= allocator->AllocatorCapacity)
        {
            alloc_addr = allocator->BaseAddress + allocator->NextAllocation;
            allocator->NextAllocation += alloc_size;
        }
    }
    LeaveCriticalSection(&allocator->AllocatorLock);
    return alloc_addr;
}
...
)
{
    ...
    {   // allocate a buffer for the file contents. 
        void  *data_buf = NULL;
        if   ((data_buf = AllocateFileBuffer(result->FileSize, shared->Allocator)) == NULL)
        ...
        if (req != NULL)
        {
            ...
            req->DataAmount  =(int64_t) CalculateAllocatorSize(result->FileSize);
            req->BaseOffset  = 0;
            req->FileOffset  = 0;
...
)
{
    ...
    {   // allocate the 'work queue' - pointers to loaded JPEG-compressed data.
        ...
        if (CreateUncachedIoAllocator(&allocator, CalculateFileDataLoadSize(file_list)) < 0)
        {
            ConsoleError("ERROR: %S(%u): Failed to pre-allocate storage for file data.\n", __FUNCTION__, GetCurrentThreadId());
            return -1;
        }
    ...
    {   // wait for an I/O slot to open up.
        ...
        if (req != NULL)
        {
            ...
            req->IoHintFlags = IO_HINT_FLAG_READ | IO_HINT_FLAG_SEQUENTIAL | IO_HINT_FLAG_UNCACHED;
